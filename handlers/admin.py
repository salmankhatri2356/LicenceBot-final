...
from config import (SUPER_ADMIN_ID, AA_NAME, AA_PHONE, AA_TID, AA_RATE, AA_SHEET,
...
async def aa_tid(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"TID: {raw}\n\nStep 4/5 — Rate per App (Rs mein, sirf number):")
...
async def aa_rate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Step 5/5 — Google Sheet ka naam daalo:\n\n"

        f"*(Jo sheet aapne manually banai hai uska exact naam — spaces bhi same)*",
...
async def aa_sheet(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Saving agent... please wait.")



    # Verify sheet exists and setup tabs

    from db import setup_manual_sheet
    ...
    if not ok_sheet:
        ...
        await update.message.reply_text(
            ...
            f"1. Sheet ka naam exactly same hai?\n"

            f"2. Service account ko Editor access diya?\n"

            f"3. Sheet Google Drive mein exist karti hai?\n\n"
